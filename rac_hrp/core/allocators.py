...
# --------------------------------------------------------------------------
# HRP -- equal-volatility variant  (Molyboga 2020, component (ii) only)
# --------------------------------------------------------------------------
# This implements ONLY the second of Molyboga's three MHRP modifications:
# "improves diversification ... by relying on an equal volatility, rather than an
#  inverse variance, allocation approach."
#
# Molyboga's other two modifications are NOT implemented here and are logged as
# out of scope for this variant:
#   (i)  exponentially weighted covariance + Ledoit-Wolf -- deferred; the decay
#        parameter is not available from the abstract and the full paper is not
#        yet obtained, so it is not guessed (pre-registration discipline).
#   (iii) volatility targeting -- excluded; it is a leverage overlay that would
#        break comparability with the equal-gross-exposure baselines.
#
# Two changes vs canonical HRP, both flowing from "equal volatility":
#   * within-cluster weights proportional to 1/sigma (not 1/sigma^2)
#   * the split fraction uses cluster VOLATILITY sqrt(v), not variance v
def _inverse_vol_weights(cov: np.ndarray, idx: Sequence[int]) -> np.ndarray:
    sig = np.sqrt(np.maximum(np.diag(cov)[idx], 1e-16))
    iv = 1.0 / sig
    return iv / iv.sum()


def _cluster_vol(cov: np.ndarray, idx: Sequence[int]) -> float:
    """Volatility of an equal-vol-weighted sub-cluster."""
    idx = np.asarray(idx, dtype=int)
    sub = cov[np.ix_(idx, idx)]
    w = _inverse_vol_weights(cov, idx)
    return float(np.sqrt(max(w @ sub @ w, 0.0)))


def hrp_equalvol_weights(cov: np.ndarray, leaf_order: np.ndarray, **_) -> np.ndarray:
    """Recursive bisection with equal-volatility (not inverse-variance) budgeting.

    Molyboga (2020) MHRP component (ii), in isolation. Same leaf order, same
    tree, same recursion as canonical HRP -- only the risk-budget split rule and
    the within-cluster weighting differ.
    """
    n = cov.shape[0]
    order = np.asarray(leaf_order, dtype=int)
    if len(order) != n:
    ...
    while clusters:
        ...
        for c in clusters:
            ...
            half = len(c) // 2
            left, right = c[:half], c[half:]
            # split on VOLATILITY, not variance: allocate less to the more
            # volatile sub-cluster, in volatility (not variance) units.
            sl, sr = _cluster_vol(cov, left), _cluster_vol(cov, right)
            alpha = 1.0 - sl / (sl + sr) if (sl + sr) > 0 else 0.5
            w[left] *= alpha
            w[right] *= (1.0 - alpha)
            nxt.extend([left, right])
        clusters = nxt
