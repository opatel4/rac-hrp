...
**Status:** rev.7 — FROZEN. Hashed and committed before any Phase 2E code was written.
**Author:** Om Patel
...
**Note on the revision count.** This specification required four substantive corrections between its
first freeze and its first power result: unfilled seeds (rev.4), an eligible-set contraction
inconsistent with the gate (rev.5), a base-path generator whose properties made the specified
comparison undefined (rev.6), and an overclaim about what the placement comparison isolates (rev.7).
Each was found before the affected result existed and each is recorded with its reason. The
manuscript reports this count, because it is evidence about the difficulty of prospective
specification rather than an embarrassment to be minimised: three of the four defects were invisible
until implementation forced a reading of code the specification had assumed.
...
1. Take the observed VI series at the 233 eligible rebalances as the base path, and centre it by
   subtracting its median so that the base path carries no clustering-change effect. Dependence and
   ordering are preserved exactly; only the location is removed.
2. Plant δ **only** at the positions belonging to the target candidate g, under the placement
   condition below. The other three candidates remain null on this same path.
...
**What the difference does and does not isolate.** The frozen inference selects its Politis–White
block length from the series it is given, and that series is the planted path. Under condition R the
planted effect follows the observed burst structure and raises the path's apparent persistence;
under condition U it does not. The block length selected can therefore differ between conditions as
a consequence of the planting.

Power_R − Power_U accordingly reflects the combined effect of clustered event placement on two
things: the difference-of-medians statistic, and the block length the procedure selects for it. Both
are real consequences of burstiness under the alternative — an effect that genuinely arrived in
bursts would produce both — so the quantity is meaningful and is the one a researcher facing bursty
events would experience. It is **not** a decomposition of the burstiness cost into a statistic
component and an inference component, and it is not reported as one.
...
At δ = 0 no effect is planted anywhere, so the target index is immaterial; the placement condition
still determines which positions are designated, but with δ = 0 the designation has no effect on the
path. δ = 0 is therefore run **once**, at 2,000 replications and B = 2,000.

**This check no longer compares against the published empirical-size figure.** That figure (0.0660,
MC SE 0.0056) was measured on i.i.d. normal VI with i.i.d. Bernoulli firing. The base path here is
the observed, dependent VI series, so the two are not comparable and any agreement or disagreement
between them would be uninterpretable. The δ = 0 cell is instead reported as the empirical size of
the frozen procedure **on the real dependence structure**, which is a quantity the project has not
previously measured and which is informative in its own right. No pass/fail rule is attached to it;
it is reported.
...
**One abort condition.** If the δ = 0 rate exceeds 0.20, the procedure is not approximately
calibrated on this dependence structure and the power curve built on it would be uninterpretable.
The run aborts and the fact is reported. This threshold is set here, before the cell is computed, at
a level far from any plausible correct value; it is a guard against gross malfunction, not a
calibration test.
